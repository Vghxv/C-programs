...
double Sp(square_t *a){return a->edge * 4;}
double Rp(rectangle_t *a){return (a->length + a->width)*2;}
double Cp(circle_t *a){return 2 * a->radius * 3.14159;}
...
double Ca(circle_t *a){return a->radius * a->radius *3.14159;}
int main(){
	int n,i;
	char cc;
	double perimeter=0;
	...
	for(i=0;i<n;i++){
		...
		perimeter+=d1;
		area+=d2;
		printf("%.2f %.2f\n",double(int(d1*100))/100,double(int(d2*100))/100);
	}
	printf("%.2f %.2f\n",double(int(perimeter*100))/100,double(int(area*100))/100);
	free(list);
	return 0;
}